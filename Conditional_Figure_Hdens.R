...
#8 unique combinations of 1s and 0s possible (3 interacting species)
...
for(i in 1:nrow(b)){
  for(j in 1:length(hd)){
    ...
    ######################## Step 6 ###############################
    #Calculate the probabilities of each unique combination of
    #1s and 0s
...
######################### Step 7 ###############################
    #Calculate Psi: Divide the probability of each combination
    #of 1s and 0s
    #by the sum of the probabilities over all combinations
    psi[, i, j] <- n / sum(n)
